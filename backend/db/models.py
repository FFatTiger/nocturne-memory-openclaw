...
class RecallDocument(Base):
    """Derived embedding document for semantic recall per reachable URI."""
    ...
    cue_text = Column(Text, nullable=False, default="")
...
class SessionReadNode(Base):
    """Per-session record of which Nocturne URIs were already opened."""

    __tablename__ = "session_read_nodes"
    ...
    first_read_at = Column(DateTime, default=datetime.utcnow)
