...
const DEFAULT_RECALL_MIN_DISPLAY_SCORE = 0.6;
...
async function fetchJson(pluginCfg, path, options = {}) {
  const url = `${pluginCfg.baseUrl}${path}`;
...
export default function register(api) {
  ...
  api.registerTool({
    name: "nocturne_list_review_groups",
    label: "Nocturne list review groups",
    description: "List pending review groups from Nocturne review API.",
    parameters: { type: "object", additionalProperties: false, properties: {} },
    async execute() {
      try {
        const data = await fetchJson(pluginCfg, "/review/groups", { method: "GET" });
        const text = Array.isArray(data) && data.length > 0
          ? data.map((item) => `- ${item.display_uri} | ${item.action} | ${item.top_level_table} | rows=${item.row_count}`).join("\n")
          : "No pending review groups.";
        return textResult(text, { ok: true, groups: data });
      } catch (error) {
        return textResult(`Nocturne review groups failed: ${error.message}`, { ok: false, error: error.message });
      }
    },
  });

  api.registerTool({
    name: "nocturne_get_review_diff",
    label: "Nocturne get review diff",
    description: "Get review diff details for a Nocturne review group by node UUID.",
    parameters: {
      type: "object",
      additionalProperties: false,
      required: ["node_uuid"],
      properties: { node_uuid: { type: "string" } }
    },
    async execute(_id, params) {
      const nodeUuid = String(params?.node_uuid || "").trim();
      try {
        const data = await fetchJson(pluginCfg, `/review/groups/${encodeURIComponent(nodeUuid)}/diff`, { method: "GET" });
        const lines = [
          `Node: ${nodeUuid}`,
          `Action: ${data?.action || ""}`,
          `Change type: ${data?.change_type || ""}`,
          data?.active_paths?.length ? `Active paths: ${data.active_paths.join(", ")}` : "",
          data?.path_changes?.length ? `Path changes: ${data.path_changes.map((x) => `${x.action}:${x.uri}`).join(", ")}` : "",
          data?.glossary_changes?.length ? `Glossary changes: ${data.glossary_changes.map((x) => `${x.action}:${x.keyword}`).join(", ")}` : "",
          "",
          "Before:",
          data?.before_content || "(empty)",
          "",
          "After:",
        ...
        return textResult(`Nocturne review diff failed: ${error.message}`, { ok: false, error: error.message, node_uuid: nodeUuid });
      }
    },
  });

  api.registerTool({
    name: "nocturne_rollback_group",
    label: "Nocturne rollback group",
    description: "Rollback a pending Nocturne review group by node UUID.",
    parameters: {
      type: "object",
      additionalProperties: false,
      required: ["node_uuid"],
    ...
    async execute(_id, params) {
      const nodeUuid = String(params?.node_uuid || "").trim();
      try {
        const data = await fetchJson(pluginCfg, `/review/groups/${encodeURIComponent(nodeUuid)}/rollback`, { method: "POST", body: JSON.stringify({}) });
        return textResult(`Rolled back review group ${nodeUuid}`, { ok: true, result: data });
      } catch (error) {
        return textResult(`Nocturne rollback failed: ${error.message}`, { ok: false, error: error.message, node_uuid: nodeUuid });
      }
    },
  });

  api.registerTool({
    name: "nocturne_list_orphans",
    label: "Nocturne list orphans",
    description: "List orphan memories from Nocturne maintenance API.",
    parameters: { type: "object", additionalProperties: false, properties: {} },
    async execute() {
      try {
        const data = await fetchJson(pluginCfg, "/maintenance/orphans", { method: "GET" });
        const text = Array.isArray(data) && data.length > 0
          ? data.map((item) => `- memory_id=${item.memory_id ?? item.id} deprecated=${item.deprecated} uri=${item.uri ?? item.path ?? ""}`).join("\n")
          : "No orphan memories.";
        return textResult(text, { ok: true, orphans: data });
      } catch (error) {
        return textResult(`Nocturne list orphans failed: ${error.message}`, { ok: false, error: error.message });
      }
    },
  });

  api.registerTool({
    name: "nocturne_get_orphan",
    label: "Nocturne get orphan",
    description: "Get orphan memory details by memory ID.",
    parameters: {
      type: "object",
      additionalProperties: false,
      required: ["memory_id"],
    ...
    async execute(_id, params) {
      ...
      try {
        const data = await fetchJson(pluginCfg, `/maintenance/orphans/${memoryId}`, { method: "GET" });
        return textResult(JSON.stringify(data, null, 2), { ok: true, orphan: data });
      } catch (error) {
        return textResult(`Nocturne get orphan failed: ${error.message}`, { ok: false, error: error.message, memory_id: memoryId });
      }
    },
  });

  api.registerTool({
    name: "nocturne_delete_orphan",
    label: "Nocturne delete orphan",
    description: "Permanently delete an orphan memory by memory ID.",
    parameters: {
      type: "object",
      additionalProperties: false,
      required: ["memory_id"],
    ...
    async execute(_id, params) {
      const memoryId = Number(params?.memory_id);
      try {
        const data = await fetchJson(pluginCfg, `/maintenance/orphans/${memoryId}`, { method: "DELETE" });
        return textResult(`Deleted orphan memory ${memoryId}`, { ok: true, result: data });
      } catch (error) {
        return textResult(`Nocturne delete orphan failed: ${error.message}`, { ok: false, error: error.message, memory_id: memoryId });
      }
    },
  });
